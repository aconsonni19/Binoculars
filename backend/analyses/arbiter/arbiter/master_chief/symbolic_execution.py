...
from backend.analyses.arbiter.target import *
from archinfo import Endness
from .sa_base import StaticAnalysis
from backend.analyses.arbiter.utils import Utils, FatalError
...
class SymExec(StaticAnalysis, DerefHook):
    ...
    def _eliminate_false_positives(self, expr, init_val, state):
        ...
        # present in the expr
        try:
            children = [x for x in set(expr.children_asts()) if x.symbolic]
    ...
    def _check_state(self, state, site, target=None, obj=None):
        ...
        if len(sym_vars) > 1:
            filtered_sym_vars = []
            try:
                for child in list(set(new_expr.leaf_asts())):
